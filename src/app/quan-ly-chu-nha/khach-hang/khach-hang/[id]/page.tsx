...
export default function CustomerEditPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        router.replace('/quan-ly-chu-nha/khach-hang/khach-hang');
  ...
  const onSubmit = async (data: FormData) => {
    try {
      if (!isEdit) {
        ...
        if (data.email) {
          try {
            ...
            // ignore search errors, let server validate on create
        ...
        if (data.phone) {
          try {
            ...
            console.warn('Phone uniqueness check failed', err);
          }
        }
        ...
        // Check backend response payload to confirm creation succeeded
        ...
        // If backend returned a message (e.g., duplicate), show it as info and do not navigate
      ...
      // If server returned a message (even with 400), show it to the user as a success-like notification
      if (serverMsg) {
  ...
  // handleSave similar to other danh-muc pages: validate then submit, show errors via toast
  const handleSave = async () => {
    try {
      const valid = await ((): Promise<boolean> => {
        // trigger minimal client validation via react-hook-form's resolver (use handleSubmit for final submit)
      ...
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-slate-200">
        <div className="max-w-screen-xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => router.push('/quan-ly-chu-nha/khach-hang/khach-hang')} className="px-3 py-2 rounded-lg border border-slate-200 hover:bg-slate-50">Hủy</button>
          </div>
        </div>
      </div>
