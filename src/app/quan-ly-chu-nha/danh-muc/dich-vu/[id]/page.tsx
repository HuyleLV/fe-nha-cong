...
export default function ServiceEditPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        router.replace('/quan-ly-chu-nha/danh-muc/dich-vu');
  ...
  const onSubmit = async (data: ServiceItem) => {
    try {
      ...
      // Coerce buildingId to number when provided
      ...
      if (!isEdit) {
        ...
        if (created && (created as any).id) {
          ...
          router.push('/quan-ly-chu-nha/danh-muc/dich-vu');
          return;
        }
      ...
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-slate-200">
        <div className="max-w-screen-xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => router.push('/quan-ly-chu-nha/danh-muc/dich-vu')} className="px-3 py-2 rounded-lg border border-slate-200 hover:bg-slate-50">Hủy</button>
          </div>
        </div>
      </div>
