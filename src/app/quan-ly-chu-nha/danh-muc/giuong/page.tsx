  ...
  return (
    <div className="p-6">
      <Panel title="Quản lý Giường" actions={(
        <Link href="/quan-ly-chu-nha/danh-muc/giuong/create" className="inline-flex items-center gap-2 bg-emerald-600 text-white px-3 py-2 rounded-md" title="Thêm giường"><PlusCircle className="w-5 h-5"/></Link>
      )}>
        <AdminTable headers={["Mã","Tên giường","Giá thuê","Đặt cọc","Trạng thái","Hành động"]}>
          {items.length === 0 ? (
            ...
            <tr key={it.id} className="border-b">
              ...
              <td className="py-3 text-sm text-slate-700">{(function(s){
                if(!s) return '-';
              ...
              <td className="py-3 text-sm text-slate-700 text-center">
                <div className="inline-flex items-center gap-2">
                  <Link href={`/quan-ly-chu-nha/danh-muc/giuong/${it.id}`} className="inline-flex items-center justify-center p-2 rounded-md bg-emerald-600 text-white hover:bg-emerald-700" title="Sửa"><Edit3 className="w-4 h-4"/></Link>
