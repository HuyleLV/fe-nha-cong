...
function HostBuildingCalendarInner() {
    ...
    (async () => {
      try {
        const [b, v] = await Promise.all([
          ...
          viewingService.hostList({ page: 1, limit: vLimit, buildingId: bid }),
        ]);
        setBuilding(b);
        setViewings(v.items || []);
      } catch (e: any) {
  ...
  return (
    <div className="max-w-7xl mx-auto p-4">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex items-center gap-2">
          <Link href="/quan-ly-chu-nha/danh-muc/toa-nha" className="px-3 py-2 rounded-lg border border-slate-200 hover:bg-slate-50">Quay lại danh sách</Link>
          <Link href={`/quan-ly-chu-nha/danh-muc/toa-nha/${bid}`} className="px-3 py-2 rounded-lg bg-emerald-600 text-white hover:bg-emerald-700">Xem tòa nhà</Link>
        </div>
      </div>
          ...
          <div className="relative bg-white rounded-xl shadow-xl border border-slate-200 w-full max-w-lg mx-4">
            ...
            <div className="p-4 space-y-3 text-sm text-slate-700">
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="col-span-2">#{selectedViewing.apartmentId}</div>
