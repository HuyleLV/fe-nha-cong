...
export default function YeuCauBaoHanhPage() {
  ...
  const load = async (p: number = 1) => {
    setLoading(true);
    try {
      const res = await reportService.getAll({ page: p, limit, type: 'warranty' });
      setItems(res.items || []);
      setMeta(res.meta || {});
      setPage(p);
  ...
  return (
    <div className="p-6">
      <Panel title="Yêu cầu bảo hành" actions={(
        <Link href="/quan-ly-chu-nha/cu-dan/yeu-cau-bao-hanh/create" className="inline-flex items-center gap-2 bg-emerald-600 text-white p-2 rounded-md" title="Tạo yêu cầu"><PlusCircle className="w-5 h-5" /></Link>
      )}>
        <p className="text-sm text-slate-600 mb-4">Danh sách yêu cầu bảo hành từ cư dân.</p>

        <AdminTable headers={["Mã","Tiêu đề","Căn hộ","Người tạo","Giá","Thời gian","Trạng thái","Hành động"]}>
          {items.length === 0 ? (
            <tr><td colSpan={8} className="py-6 text-center text-slate-500">{loading ? 'Đang tải...' : 'Chưa có yêu cầu bảo hành'}</td></tr>
          ) : items.map((it, idx) => (
            <tr key={it.id} className="border-b">
              ...
              <td className="py-3 text-sm text-slate-700 text-center">
                <div className="inline-flex items-center gap-2">
                      <Link href={`/quan-ly-chu-nha/cu-dan/yeu-cau-bao-hanh/${it.id}`} className="inline-flex items-center justify-center p-2 rounded-md bg-emerald-600 text-white hover:bg-emerald-700" title="Xem"><Eye className="w-4 h-4"/></Link>
                      <>
