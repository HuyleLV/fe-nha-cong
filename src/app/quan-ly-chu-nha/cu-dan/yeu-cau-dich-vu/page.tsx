                ...
                <div className="inline-flex items-center gap-2">
                      ...
                      <>
                        ...
                        <ConfirmModal open={confirmOpen && confirmId === it.id} message={confirmMessage} onCancel={() => { setConfirmOpen(false); setConfirmId(null); }} onConfirm={async () => {
                          if (!confirmId) return; try { await serviceRequestService.remove(confirmId); toast.success('Xóa yêu cầu thành công'); await load(page); } catch (err: any) { console.error(err); toast.error(err?.response?.data?.message ?? 'Xóa thất bại'); } finally { setConfirmOpen(false); setConfirmId(null); }
                        }} />
                      </>
                </div>
              </td>
