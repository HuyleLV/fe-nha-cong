...
export default function FireReportEditPage() {
  ...
  useEffect(() => {
    (async () => {
      if (isEdit) {
        ...
        } catch (err) { console.error(err); toast.error('Không thể tải báo cháy nổ'); router.back(); }
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      if (!isEdit) {
        ...
        toast.success('Tạo báo cháy nổ thành công');
        router.push('/quan-ly-chu-nha/cu-dan/bao-chay-no');
        return;
      }

      await reportService.update(Number(id), payload);
      toast.success('Cập nhật báo cháy nổ thành công');
      router.push('/quan-ly-chu-nha/cu-dan/bao-chay-no');
    } catch (err: any) {
      console.error(err);
      ...
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-slate-200">
        <div className="max-w-screen-xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-slate-500">{isEdit ? 'Chỉnh sửa báo cháy nổ' : 'Tạo báo cháy nổ'}</p>
              <h1 className="text-lg font-semibold text-slate-800">{isEdit ? 'Chỉnh sửa báo cháy nổ' : 'Tạo báo cháy nổ'}</h1>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={() => {
                ...
                else { router.push('/quan-ly-chu-nha/cu-dan/bao-chay-no'); }
              }}
