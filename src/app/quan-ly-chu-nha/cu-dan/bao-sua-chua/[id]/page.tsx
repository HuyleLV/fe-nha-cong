...
export default function RepairEditPage() {
  ...
  const { register, handleSubmit, reset, watch, setValue, formState: { errors, isSubmitting } } = useForm<any>({ defaultValues: { title: '', description: '', amount: undefined, reportedAt: '', status: 'pending', buildingId: undefined, apartmentId: undefined, customerId: undefined } });
  ...
  useEffect(() => {
    (async () => {
      if (isEdit) {
        ...
        } catch (err) { console.error(err); toast.error('Không thể tải báo sửa chữa'); router.back(); }
  ...
  const onSubmit = async (data: any) => {
    try {
      ...
      if (!isEdit) {
        ...
        toast.success('Tạo báo sửa chữa thành công');
        router.push('/quan-ly-chu-nha/cu-dan/bao-sua-chua');
        return;
      }

      await reportService.update(Number(id), payload);
      toast.success('Cập nhật báo sửa chữa thành công');
      router.push('/quan-ly-chu-nha/cu-dan/bao-sua-chua');
    } catch (err: any) {
      console.error(err);
      ...
      <div className="sticky top-0 z-30 bg-white/80 backdrop-blur border-b border-slate-200">
        <div className="max-w-screen-xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-slate-500">{isEdit ? 'Chỉnh sửa báo sửa chữa' : 'Tạo báo sửa chữa'}</p>
              <h1 className="text-lg font-semibold text-slate-800">{isEdit ? 'Chỉnh sửa báo sửa chữa' : 'Tạo báo sửa chữa'}</h1>
            </div>
          </div>
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              onClick={() => {
                ...
                else { router.push('/quan-ly-chu-nha/cu-dan/bao-sua-chua'); }
              }}
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="grid grid-cols-1 gap-6 p-4">
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-sm text-slate-600 mb-1">Giá dự kiến (nếu có)</label>
              <input type="number" className={inputCls} {...register('amount')} />
            </div>
