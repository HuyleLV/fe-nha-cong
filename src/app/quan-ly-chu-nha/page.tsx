...
export default function HostDashboardPage() {
  ...
  useEffect(() => {
    ...
    // Fetch owner-scoped stats in parallel. axiosClient will attach the host token
    // when running inside /quan-ly-chu-nha so backend should scope results to the current host.
    Promise.allSettled([
      buildingService.getAll({ limit: 1 }),
      apartmentService.getAll({ limit: 1 }),
      contractService.list({ limit: 1 }),
      // depositService may be admin-scoped in some backends; try list and fallback to 0
