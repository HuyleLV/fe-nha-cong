  ...
  return (
    <div className="p-6">
      <Panel title="Ghi chỉ số">
          ...
          <p className="text-sm text-slate-600">Danh sách ghi chỉ số (Admin).</p>
          <div>
            <button onClick={() => router.push('/admin/tai-chinh/ghi-chi-so/create')} className="inline-flex items-center gap-2 bg-emerald-600 text-white px-3 py-2 rounded-md" title="Thêm mới"><PlusCircle className="w-5 h-5"/></button>
          </div>
        </div>
            ...
            <tr key={r.id} className="border-t">
              ...
              <td className="px-4 py-3 text-center">
                <div className="flex items-center justify-center gap-2">
                  <button title="Sửa" onClick={() => router.push(`/admin/tai-chinh/ghi-chi-so/${r.id}`)} className="p-2 rounded bg-emerald-600 text-white">
