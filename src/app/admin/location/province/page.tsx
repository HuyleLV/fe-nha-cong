...
function LevelTag({ level }: { level: LocationLevel }) {
  ...
  return (
    <span className={`inline-block border px-2 py-0.5 rounded-full text-xs ${map[level]}`}>{level}</span>
  );
}

export default function ProvinceLocationPage() {
  const router = useRouter();
  const level: LocationLevel = "Province";
  ...
  return (
    <div className="p-4">
      <div className="flex items-center justify-between gap-2">
        <h1 className="text-xl font-semibold text-emerald-900">ĐỊA ĐIỂM · TỈNH</h1>
        <button
          ...
          onClick={() => router.push("/admin/location/create?level=Province")}
        >
          <Plus size={16} /> Tạo Tỉnh
        </button>
      </div>
      ...
      <table className="w-full text-left border border-gray-200 shadow rounded-lg overflow-hidden mt-5">
        ...
        <tbody className="divide-y divide-gray-200 text-[15px]">
              ...
              <td className="px-4 py-2">
                <div className="flex justify-center gap-2">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      const ok = confirm(`Xoá Tỉnh "${loc.name}"?`);
                      if (!ok) return;
                      try {
          ...
          {!items.length && (
            <tr>
              <td colSpan={7} className="px-4 py-6 text-center text-gray-500">Chưa có Tỉnh.</td>
            </tr>
          )}
        </tbody>
      </table>
